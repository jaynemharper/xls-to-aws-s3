...
path = 'INSERT PATH TO FOLDER USED AS HOMEBASE'
...
for filename in glob.glob(os.path.join(start_folder, '*INSERT FILE1 & FILE2 NAME IDENTIFIER*')):
    if fnmatch.fnmatch(filename, '*INSERT FILE1 NAME IDENTIFIER*'):
        ...
        pd.to_datetime(df1['INSERT COLUMN_NAME TO BE REFORMATTED TO DATETIME'], format="%Y-%m-%d %H:%M:%S.%f") #use format that matches your data
        ...
        pd.to_datetime(df2['INSERT COLUMN_NAME TO BE REFORMATTED TO DATETIME'], format='%Y-%m-%d %H:%M:%S.%f') #use format that matches your data
...
for filename in glob.glob(os.path.join(middle_folder, '*INSERT FILE1 & FILE2 NAME IDENTIFIER*')):
